...
class AgentState(TypedDict, total=False):
    ...
    # Student-added fields. All four are OVERWRITE (not append-only): each node
    # cares only about the *latest* value, never the history. Using `add` here
    # would concatenate strings/dicts and break routing reads.
    evaluation_result: str  # "success" | "needs_retry" — gates route_after_evaluate
    pending_question: str  # clarification text for missing_info / rejected-risky flow
    proposed_action: str  # description of the risky action awaiting approval
    approval: dict[str, Any] | None  # ApprovalDecision dump — read by route_after_approval
    # Append-only audit channels (reducer = operator.add).
