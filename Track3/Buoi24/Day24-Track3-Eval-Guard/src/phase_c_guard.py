...
def setup_presidio():
    ...
    # Dùng model sẵn có — lg nếu có, fallback về sm
...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Chỉ scan các loại PII liên quan — tránh spaCy NLP false-positive trên text tiếng Việt
...
_JAILBREAK_PATTERNS = [
    ...
    "bỏ qua tất cả", "bỏ qua hướng dẫn", "ignore your", "ignore all",
    "ignore previous", "forget your", "forget all instructions",
    "system prompt", "system override", "system instructions",
    "override", "bypass",
    # Role-play jailbreak
    "pretend you are", "pretend you're", "you are now", "you are an",
    "act as", "đóng vai", "không có giới hạn",
    "unrestricted", "no restrictions", "dan ", "jailbreak",
    # PII / data extraction requests
    "cho tôi biết cccd", "tiết lộ thông tin", "tiết lộ bảng lương",
    "bảng lương chi tiết", "thông tin cá nhân của nhân viên",
    "số điện thoại của nhân viên", "dump all", "liệt kê toàn bộ thông tin",
    "toàn bộ thông tin nhân viên", "in ra toàn bộ", "employee records",
    "training data",
    # Malicious requests
    "tấn công mạng", "hack", "malware", "mật khẩu admin", "mật khẩu hệ thống",
    "hướng dẫn tấn công",
    # HTML/special injection markers
    "<!-- ignore", "<!--", "[admin command", "[system:", "[priority 1]",
    "admin command",
    # Off-topic topics
    "viết một bài thơ", "bài thơ", "nấu phở", "nấu ăn", "công thức nấu",
    "giá bitcoin", "giá ethereum", "cryptocurrency", "crypto",
    "phương trình vi phân", "giải phương trình", "toán học",
    "bộ phim", "marvel", "netflix",
    "thời tiết hôm nay", "tin tức hôm nay",
]
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Keyword pre-screening — catch common jailbreak/PII patterns before NeMo
    text_lower = text.lower()
    for pattern in _JAILBREAK_PATTERNS:
        if pattern in text_lower:
            return {
                "allowed":        False,
                "blocked_reason": "keyword_prescreen",
    ...
    # generate_async trả về str (cũ) hoặc dict (NeMo>=0.9)
    if isinstance(raw, dict):
        response = raw.get("content", raw.get("output", str(raw)))
    else:
        response = str(raw)

    # NeMo từ chối bằng cách trả về refuse message định nghĩa trong rails.co
...
def measure_p95_latency(test_inputs: list[str], n_runs: int = 20,
    ...
    async def _measure():
        # Warmup: discard first call (model loading cold start)
        pii_scan("warmup call", analyzer, anonymizer)
...
if __name__ == "__main__":
    ...
    passed_count = sum(1 for r in adv_results if r["passed"]) if adv_results else 0
    pass_rate = passed_count / len(adversarial_set) if adversarial_set else 0.0
