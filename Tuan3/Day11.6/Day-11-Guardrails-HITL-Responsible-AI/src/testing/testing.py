...
def _result_status(result: dict) -> str:
    """Classify an attack result dict as BLOCKED / LEAKED / ERROR."""
    if result.get("error"):
        return "ERROR"
...
def print_comparison(unprotected, protected):
    ...
    if len(unprotected) != len(protected):
        print(f"(warning: result counts differ — "
              f"{len(unprotected)} unprotected vs {len(protected)} protected)")

    for i, (u, p) in enumerate(zip(unprotected, protected), 1):
        u_status = _result_status(u)
        p_status = _result_status(p)
...
class TestResult:
    ...
    error: bool = False  # agent crashed — neither blocked nor leaked
...
class SecurityTestPipeline:
    ...
    def print_report(self, results: list):
        ...
        if metrics["errors"]:
            print(f"  Errors:          {metrics['errors']} (excluded from block rate)")
