...
from core.config import ALLOWED_TOPICS, BLOCKED_TOPICS, INJECTION_PATTERNS
from core.utils import extract_text


# Compile once at import time — these run on every user message.
_INJECTION_RE = [re.compile(p, re.IGNORECASE) for p in INJECTION_PATTERNS]
# Word boundaries so "hack" doesn't match "hackathon", "vay" doesn't match "vaycation".
...
def detect_injection(user_input: str) -> bool:
    ...
    """
    return any(p.search(user_input) for p in _INJECTION_RE)
...
def topic_filter(user_input: str) -> bool:
    ...
    if any(p.search(input_lower) for p in _BLOCKED_RE):
        return True

    if any(p.search(input_lower) for p in _ALLOWED_RE):
        return False
