...
# Load .env so API keys configured there are visible to os.getenv(), whether the
# code runs via the CLI or under pytest. Guarded so the lab still works if
# python-dotenv is not installed (keys can also be exported manually).
try:
